...
def train(train_loader, model, criterion, optimizer, epoch, log):
  ...
  for i, (input, target) in enumerate(train_loader):
    ...
    import numpy as np
    all_losses = F.cross_entropy(output, target_var, reduction="none")
    #print(all_losses.shape)
    #print(all_losses)
    #print(all_losses.cpu().detach().numpy())
    #exit(0)
    ...
    if curiosity:
      # select worst items
      all_losses = all_losses.cpu().detach().numpy().reshape(BATCH_SIZE,)
    ...
    r_inputs = input[retry_idx]
    r_outputs = target[retry_idx]

    # train
    r_prediction = model(r_inputs)
    r_loss = criterion(r_prediction, r_outputs)
    optimizer.zero_grad()
    r_loss.backward()
    optimizer.step()
